...
module.exports = function(formio, items, done) {
  ...
  if (formio.config.project) {
    ...
    if (formio.config.project.user) {
      ...
      project.user.loginForm =
        formio.config.project.user.loginForm || 'user/login';
    }
  }
  ...
  const steps = {
    ...
    importTemplate: function(done) {
      ...
      try {
        info = JSON.parse(
          fs.readFileSync(path.join(directoryPath, 'package.json')),
        );
      } catch (err) {
        debug(err);
        return done(err);
        ...
        formio.resources.submission.model.create(
          {
            ...
            roles: [project.config.roles.administrator._id],
          },
          function(err, item) {
            if (err) {
              return done(err);
  ...
  util.log('Installing...');
  async.series(
    [steps.configureClient, steps.importTemplate, steps.createRootUser],
    function(err, result) {
      if (err) {
        util.log(err);
        return done(err);
